...
program.version("0.6.0").name("elm-codegen").addHelpText("before", helpText);
...
`,
  ...
  .option(
    "--cwd <dir>",
    "The directory where your elm.codegen.json lives.",
    ".",
  )
...
`,
  ...
  .option(
    "--debug",
    "Run your generator in debug mode, allowing you to use Debug.log in your elm.",
    false,
  )
  .option(
    "--watch",
    "Watch the given file for changes and rerun the generator when a change is made.",
    false,
  )
  .option(
    "--output <dir>",
  ...
  .option(
    "--cwd <dir>",
    "The base directory where the elm.json lives for your generator.",
    ".",
  )
  .option(
    "--flags-from <file>",
  ...
  .option(
    "--flags <json>",
    "Json to pass to your elm app.  if --flags-from is given, that will take precedence.",
  )
